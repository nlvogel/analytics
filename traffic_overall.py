        ...
        try:
            for row in rows:
                data_list = [
                    ...
                    f'{round(float(row.get("metrics")[0].get("values")[2]), 2)}%',
                    row.get('metrics')[0].get('values')[8],
                    row.get('metrics')[0].get('values')[5],
                    row.get('metrics')[0].get('values')[6],
                    row.get('metrics')[0].get('values')[7],
                    row.get('metrics')[0].get('values')[3],
                    row.get('metrics')[0].get('values')[4]
                ]
                ...
                # sheets.append_sheet([data_list], SHEET, 'Sheet1!A1')
                with open(f'{now}-overall_traffic.csv', mode='a') as f:
                    writer = csv.writer(f)
