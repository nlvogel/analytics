            ...
            'reportRequests': [
                {
                    ...
                    'dimensions': [{'name': 'ga:segment'}, {'name': 'ga:pagePathLevel4'}],
                    'dimensionFilterClauses': [
                        {
                            'operator': 'AND',
                            'filters': [
                                {
                                    'dimensionName': 'ga:pagePathLevel4',
                                    'operator': 'REGEXP',
                                    'expressions': segment
                                },
                                {
        ...
        try:
            for row in rows:
                data_list = [
                    ...
                    region,
                    # add 'region', change below filter to data_list[2]
                    row.get('dimensions')[0],
                    row.get('dimensions')[1].split('/')[2].title(),
                ...
                # sheets.append_sheet([data_list], SHEET, 'Floor Plans!A2')
                with open(f'{now}-floor_plans.csv', mode='a') as f:
                    writer = csv.writer(f)
